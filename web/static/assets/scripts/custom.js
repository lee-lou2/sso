...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "JA2BIT"; //Local Storage Names for PWA
    var pwaRemind = 1; //Days to re-remind to add to home
    var pwaNoCache = false; //Requires server and HTTPS/SSL. Will clear cache with each visit

    //Setting Service Worker Locations scope = folder | location = service worker js location
    var pwaScope = "/";
    var pwaLocation = "/static/assets/_service-worker.js";
    ...
    function init_template(){
        ...
        highlightData.forEach(el => el.addEventListener('click', e =>{
            ...
            loadHighlight.href = 'static/assets/styles/highlights/highlight_' + highlight +'.css';
        ...
        if(rememberHighlight){
            ...
            loadHighlight.className = "page-highlight";
            loadHighlight.type = "text/css";
            loadHighlight.href = 'static/assets/styles/highlights/highlight_' + rememberHighlight +'.css';
            ...
            loadHighlight.className = "page-highlight";
            loadHighlight.type = "text/css";
            loadHighlight.href = 'static/assets/styles/highlights/highlight_' + defaultHighlight[1] +'.css';
            if(!document.querySelectorAll('.page-highlight').length){
                document.getElementsByTagName("head")[0].appendChild(loadHighlight);
        ...
        if(locationBut.length){
            ...
            if (typeof(locationSupport) != 'undefined' && locationSupport != null){
                ...
                if ("geolocation" in navigator){
                    locationSupport.innerHTML = '운동 장소에 도착하셨다면 <strong class="color-green2-dark">클릭</strong> 🐶 해주세요.';
                }else{
                    locationSupport.innerHTML = '운동 장소에 도착하셨다면 <strong class="color-red2-dark">클릭</strong> 🐶 해주세요.';
                }
            }
            function geoLocate() {
                ...
                function success(position) {
                    ...
                    var mapL1 = 'https://www.google.com/maps/embed/v1/view?key=AIzaSyALPJ1w-E1DSWilI4QCRpJsLgFJsEeV5WQ&center=';
                    var mapL2 = latitude+',';
                    var mapL3 = longitude;
                    var mapL4 = '&zoom=17&maptype=roadmap'
                    var mapLinkEmbed = mapL1 + mapL2 + mapL3 + mapL4;
					document.getElementsByClassName('location-map')[0].setAttribute('src',mapLinkEmbed);
                    document.getElementsByClassName('geo-iframe')[0].classList.remove('disabled');
                    document.getElementsByClassName('location-coordinates')[0].classList.add('disabled');
                    const csrftoken = getCookie('csrftoken');
                    fetch("/v1/exercise/is_enabled_range/", {
                      method: "POST",
                      headers: {
                      ...
                      body: JSON.stringify({
                        ...
                        if(response.status == "200") {
                            // 등록 완료
                            console.log("운동 장소 범위 내");
                            document.getElementsByClassName('location-button')[0].setAttribute("data-menu", "menu-modal-exercise-confirm");
                            document.getElementsByClassName('location-button')[0].setAttribute("onclick", "exercise_confirm();");
                        } else if(response.status == "404") {
                            // 운동 장소를 찾지 못함
                            console.log("운동 장소 범위 외");
                            document.getElementsByClassName('location-button')[0].setAttribute("data-menu", "menu-modal-exercise");
                        } else {
                            console.log("알 수 없는 오류");
                            console.log(response);
                        }
                        document.getElementsByClassName('geo-iframe')[0].setAttribute('lat', latitude.toString());
                        document.getElementsByClassName('geo-iframe')[0].setAttribute('lng', longitude.toString());
                        document.getElementsByClassName('location-button')[0].classList.remove('disabled');
                    });
                }
                function error() {locationCoordinates.textContent = '위치 조회를 실패 했어요 😭';}
                if (!navigator.geolocation) {locationCoordinates.textContent = '해당 브라우저는 위치 조회를 지원하지 않습니다 😭';}
                else {locationCoordinates.textContent = '🐎 현재 위치 조회중! 잠시만요~~~ 🐎';navigator.geolocation.getCurrentPosition(success, error);}
            }
